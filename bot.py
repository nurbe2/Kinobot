...
c.execute("CREATE TABLE IF NOT EXISTS kinolar (kod INT, nomi TEXT, tavsif TEXT, reyting REAL, tip TEXT, fayl TEXT, qism INT, janr TEXT, sana TEXT, korish INT)")
...
c.execute("CREATE TABLE IF NOT EXISTS reviews (id INTEGER PRIMARY KEY AUTOINCREMENT, kod INT, uid INT, user TEXT, matn TEXT, sana TEXT)")
c.execute("CREATE TABLE IF NOT EXISTS users (uid INT, name TEXT, username TEXT, sana TEXT)")
...
def save_user(uid, name, username):
...
def start(msg):
    ...
    states.pop(uid, None)
    save_user(uid, msg.from_user.first_name, msg.from_user.username)
...
def show_menu(uid):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("Nomi orqali", callback_data="name"),
        types.InlineKeyboardButton("Janr orqali", callback_data="genre"),
        types.InlineKeyboardButton("TOP Reyting", callback_data="topr"),
        types.InlineKeyboardButton("TOP Korilgan", callback_data="topv"),
        types.InlineKeyboardButton("NexMovie Pro", callback_data="pro"),
        types.InlineKeyboardButton("Statistika", callback_data="stats")
    )
    bot.send_message(uid, "Menu | " + p, reply_markup=markup)
...
def step1(msg):
    if not msg.text.isdigit():
        bot.send_message(msg.from_user.id, "Raqam kiriting!"); return
    states[msg.from_user.id]['data']['kod'] = int(msg.text)
    states[msg.from_user.id]['step'] = 2
    bot.send_message(msg.from_user.id, "Kino nomi:")
...
def step4(msg):
    try:
        r = float(msg.text)
        if r < 1 or r > 10:
            bot.send_message(msg.from_user.id, "1-10 gacha!"); return
    except:
...
def step5(msg):
    if msg.photo:
        ...
        bot.send_message(msg.from_user.id, "Rasm yoki video yuboring!"); return
...
def step6(msg):
    ...
    bot.send_message(msg.from_user.id, "Janr (vergul bilan):")
...
def q2(msg):
    ...
    bot.send_message(msg.from_user.id, "Video yuboring:")
...
def send_kino(uid, k, view=True):
    if view:
        c.execute("UPDATE kinolar SET korish = korish + 1 WHERE kod=?", (k[0],))
        conn.commit()
    ...
    markup.add(
        types.InlineKeyboardButton("1", callback_data="r_" + str(k[0]) + "_1"),
        types.InlineKeyboardButton("2", callback_data="r_" + str(k[0]) + "_2"),
        types.InlineKeyboardButton("3", callback_data="r_" + str(k[0]) + "_3"),
        types.InlineKeyboardButton("4", callback_data="r_" + str(k[0]) + "_4"),
        types.InlineKeyboardButton("5", callback_data="r_" + str(k[0]) + "_5")
    )
    ...
    markup.add(
        types.InlineKeyboardButton("Fikr", callback_data="rev_" + str(k[0])),
        types.InlineKeyboardButton("Fikrlar", callback_data="revs_" + str(k[0]))
    )
...
@bot.callback_query_handler(func=lambda c: c.data == "name")
...
def show_kino_name(msg):
    uid = msg.from_user.id
    name = msg.text.strip()
    c.execute("SELECT * FROM kinolar WHERE nomi LIKE ?", ('%' + name + '%',))
    kinolar = c.fetchall()
    if not kinolar:
        bot.send_message(uid, "Topilmadi!"); states.pop(uid, None); return
    states.pop(uid, None)
...
@bot.callback_query_handler(func=lambda c: c.data.startswith('rev_') and not c.data.startswith('revs_'))
def review_start(call):
    kod = int(call.data.split('_')[1])
    states[call.from_user.id] = {'step': 'review', 'kod': kod}
    bot.answer_callback_query(call.id)
    bot.send_message(call.from_user.id, "Fikringiz:")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith('revs_'))
def show_reviews(call):
    kod = int(call.data.split('_')[1])
    c.execute("SELECT * FROM reviews WHERE kod=? ORDER BY id DESC LIMIT 10", (kod,))
    revs = c.fetchall()
    if not revs:
        bot.answer_callback_query(call.id, "Fikrlar yoq!", show_alert=True); return
    text = "Fikrlar:\n\n"
    for r in revs:
        text += r[3] + ": " + r[4] + "\n---\n"
...
@bot.callback_query_handler(func=lambda c: c.data == "topr")
def top_rating(call):
    if not check_sub(call.from_user.id):
        bot.answer_callback_query(call.id, "Obuna boling!", show_alert=True); return
...
@bot.callback_query_handler(func=lambda c: c.data == "topv")
def top_views(call):
    if not check_sub(call.from_user.id):
        bot.answer_callback_query(call.id, "Obuna boling!", show_alert=True); return
    c.execute("SELECT * FROM kinolar ORDER BY korish DESC LIMIT 10")
    kinolar = c.fetchall()
    markup = types.InlineKeyboardMarkup(row_width=1)
    for i, k in enumerate(kinolar, 1):
...
def stats(call):
    ...
    c.execute("SELECT COUNT(*) FROM kinolar"); k = c.fetchone()[0]
...
def full_stats(msg):
    ...
    c.execute("SELECT COUNT(*) FROM pro"); p = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM reviews"); r = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM ratings"); rt = c.fetchone()[0]
    text = "Users:" + str(u) + " Kino:" + str(k) + " Views:" + str(v) + " PRO:" + str(p) + " Reviews:" + str(r) + " Rates:" + str(rt)
...
def pro_menu(call):
    ...
    if is_pro(call.from_user.id):
        bot.answer_callback_query(call.id, "PROsiz!", show_alert=True); return
    markup = types.InlineKeyboardMarkup()
